...
import IEmojiGroup from "./IEmojiGroup.vue";
import IInput from "@/components/input/IInput.vue";
import type {
	EmojiItem,
	EmojiCategoryData as CategoryData,
	EmojiFilteredCategory as FilteredCategory,
} from "./IEmoji.types";
...
const props = withDefaults(
	defineProps<{
		/**
		 * Controls picker visibility (for animations/loading)
		 */
		isShow?: boolean;
	}>(),
	{
...
// Refs - following widget-components structure
const emojipicker = ref<HTMLDivElement>();
const scroll = ref<HTMLDivElement>();
const searchInput = ref("");
const activeCategory = ref<FilteredCategory | { category_name: string; name: string }>();
const categoryRefs = ref<HTMLDivElement[]>([]);
const isSearch = ref(false);
const search = ref<EmojiItem[] | null>(null);
const loaded = ref(false);
const emojione = ref<any>(null);
const shortnames = ref<string[] | null>(null);
const checkTimeout = ref<number | null>(null);
const counter = ref(0);
const filteredCategories = ref<FilteredCategory[]>([]);
const waitingForPaint = ref(false);
const placeholder = ref("Search Emoji");

// Emoji categories data - following widget-components structure exactly
const categories = ref<Record<string, CategoryData>>({
	'people': {
		'name': 'Smileys & People',
		'emoji': '1f603',
		'header': ':smiley:',
...
// Methods - following widget-components structure
const includeScript = () => {
...
const checkScriptLoaded = () => {
	...
	setTimeout(() => {
		...
		emojione.value.ascii = true;

		shortnames.value = emojione.value.shortnames
			.replace(/\\\+/g, '+')
			.split('|');

		Object.keys(categories.value).forEach((key) => {
			const category = categories.value[key]!;
			const pool: FilteredCategory = {
				category_name: key,
				emoji: category.emoji,
				name: category.name,
				header: emojione.value.shortnameToImage(category.header),
				emojis: [],
				show: false
			};

			category.content.forEach((shortname) => {
				pool.emojis.push({
					title: shortname
				});
			});

			filteredCategories.value.push(pool);
		});

		filteredCategories.value[0]!.show = true;
		activeCategory.value = filteredCategories.value[0];
		loaded.value = true;
...
const onKeyup = () => {
	...
	if (text !== '') {
		...
		if (shortnames.value) {
			for (let i = 0; i < shortnames.value.length; i++) {
				const index = shortnames.value[i]!;

				if (index.indexOf(text) !== -1) {
					results.push({
						title: index
					});
				}
...
const onGroupClick = (category: FilteredCategory) => {
	const index = filteredCategories.value.indexOf(category);
	const component = categoryRefs.value[index];

	if (component) {
		let top = component.offsetTop + 15;
	...
	filteredCategories.value[index]!.show = true;
	activeCategory.value = filteredCategories.value[index];
};

const onClick = (event: Event) => {
	const target = event.target as HTMLElement;
	if (emojione.value) {
		const emoji = emojione.value.shortnameToUnicode(target.title);
		emit('select', emoji);
	}
};
...
const onScrollPaint = () => {
	...
	let newActiveCategory = filteredCategories.value[0];
	...
	for (let i = 0, l = filteredCategories.value.length; i < l; i++) {
		...
		filteredCategories.value[i]!.show = true;
...
onMounted(() => {
	nextTick(() => {
		...
		// If isShow is true on mount, start checking for script
		if (props.isShow) {
			checkScriptLoaded();
		}
	});
...
<template>
	<div
		class="i-emoji-picker i-outline"
		ref="emojipicker"
		tabindex="0">
		...
		<div class="i-emoji-search-container">
			<i-input
				v-model="searchInput"
				:label="placeholder"
			...
			<h4 class="i-emoji-sticky-header">
				{{ activeCategory?.name }}
			</h4>

			<div
				ref="scroll"
				class="i-emoji-scroll"
				@scroll="onScroll">

				<template v-if="isSearch">
					<button
						v-for="emoji in search"
						:key="emoji.title"
						:title="emoji.title"
						:id="emoji.title"
						class="i-emoji i-outline"
						tabindex="0"
						@click="onClick">
						<i-emoji :emoji="emoji.title" />
					</button>
				...
				<div
					...
					:key="index"
					v-show="!isSearch"
					:ref="(el) => { if (el) categoryRefs[index] = el as HTMLDivElement }">

					<h4
						v-if="index != 0"
						class="i-emoji-header">
						{{ category.name }}
					</h4>
